...
TEXTS = {
    "zh": {
        "welcome_message": f"Auto-Upgrade Script {CURRENT_SCRIPT_VERSION}\nAgent-Avatar 升级脚本 - 此脚本仍在实验阶段，可能无法按预期工作。",
        "lang_select": "请选择语言/Please select language (zh/en):",
        "invalid_lang": "无效的语言选择，使用英文作为默认语言",
        "not_git_repo": "错误：当前目录不是git仓库。请进入 Agent-Avatar 目录后再运行此脚本。\n当然，更有可能的是你下载的Agent-Avatar不包含.git文件夹 (如果你是透过下载压缩包而非使用 git clone 命令下载的话可能会造成这种情况)，这种情况下目前无法用脚本升级。",
...
是否继续？(y/N): """,
    ...
    "en": {
        "welcome_message": f"Auto-Upgrade Script {CURRENT_SCRIPT_VERSION}\nAgent-Avatar upgrade script - This script is highly experimental and may not work as expected.",
        "lang_select": "请选择语言/Please select language (zh/en):",
        "invalid_lang": "Invalid language selection, using English as default",
        "not_git_repo": "Error: Current directory is not a git repository. Please run this script inside the Agent-Avatar directory.\nAlternatively, it is likely that the Agent-Avatar you downloaded does not contain the .git folder (this can happen if you downloaded a zip archive instead of using git clone), in which case you cannot upgrade using this script.",
...
def sync_user_config(logger, lang: str = "en") -> None:
    texts = TEXTS[lang]
    default_template = (
        "config/templates/conf.ZH.default.yaml"
        if lang == "zh"
        else "config/templates/conf.default.yaml"
    )
