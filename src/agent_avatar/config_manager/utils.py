...
def scan_config_alts_directory(config_alts_dir: str) -> list[dict]:
    ...
    # Sort function: alphabetically by character name, then numerically by outfit number
    def sort_key(item):
        name = item["name"]
        # Extract character name and outfit number
        if "Outfit" in name:
            parts = name.split("Outfit")
            char_name = parts[0].strip()
            try:
                outfit_num = int(parts[1].strip())
            except:
                outfit_num = 0
            return (char_name.lower(), outfit_num)
        else:
            # No outfit number, sort alphabetically
            return (name.lower(), 0)

    config_files.sort(key=sort_key)

    # Add default config at the beginning
    if default_config_item:
        config_files.insert(0, default_config_item)
