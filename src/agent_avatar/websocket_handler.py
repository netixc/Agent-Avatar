...
class WebSocketHandler:
    ...
    async def _send_initial_messages(
        ...
        # Initialize Agent-Zero context with current history (if any)
        agent_zero_client = get_agent_zero_client()
        if agent_zero_client.enabled and session_service_context.history_uid:
            agent_zero_client.set_context_from_history(session_service_context.history_uid)
            logger.info(f"Agent-Zero context initialized with existing history: {session_service_context.history_uid}")

        # Add defaultEmotion to model_info for frontend initialization
        model_info_with_default = session_service_context.live2d_model.model_info.copy()
        model_info_with_default["defaultEmotion"] = 0  # Set neutral as default emotion

        await websocket.send_text(
            json.dumps(
                {
                    "type": "set-model-and-conf",
                    "model_info": model_info_with_default,
    ...
    async def _handle_fetch_history(
        ...
        # Sync Agent-Zero context with the selected history
        agent_zero_client = get_agent_zero_client()
        if agent_zero_client.enabled:
            agent_zero_client.set_context_from_history(history_uid)
            logger.info(f"Agent-Zero context synced to history: {history_uid}")
    ...
    async def _handle_create_history(
        ...
        if history_uid:
            ...
            # Sync Agent-Zero context with the new history
            agent_zero_client = get_agent_zero_client()
            if agent_zero_client.enabled:
                agent_zero_client.set_context_from_history(history_uid)
                logger.info(f"Agent-Zero context synced to NEW history: {history_uid}")
